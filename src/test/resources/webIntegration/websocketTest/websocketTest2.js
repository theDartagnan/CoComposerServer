...
async function sendFirstElementCreationOrder(wsClient, compoId) {
  await waitSeconds(2);
  console.log('Send element creation');
  let order = {orderType: 'elementAdded', element: {elementType: 'rect', x: 100, y: 124, style: 'color:red;', width: 32, height: 43}};
...
async function sendElementPositionChangedOrder(wsClient, compoId, elemId) {
  await waitSeconds(2);
  console.log('Send element creation');
  let order = {orderType: 'elementPositionChanged', elementId: elemId, x: 540, y: 890};
  sendCompositionOrder(wsClient, compoId, order);
}
...
function handleCompositionTopicMessage(rawMessage, wsClient, testSteps) {
  try {
    ...
    if (message.orderType === 'elementAdded' && message.authorEmail === 'mem2@collamap.com' && testSteps.step === 0) {
      console.log('current user add element in proper order');
      testSteps.step++;
    } else if (message.orderType === 'elementPositionChanged' && message.authorEmail === 'mem1@collamap.com' && testSteps.step === 1) {
      console.log('Other user moved element in proper order');
      testSteps.step++;
      sendElementPositionChangedOrder(wsClient, message.compositionId, message.elementId);
    } else if (message.orderType === 'elementPositionChanged' && message.authorEmail === 'mem2@collamap.com' && testSteps.step === 2) {
      console.log('Current user moved element in proper order');
      testSteps.step++;
    } else if (message.orderType === 'elementDeleted' && message.authorEmail === 'mem1@collamap.com' && testSteps.step === 3)  {
      console.log('Other user deleted element in proper order');
      testSteps.step++;
      waitSeconds(2).then(() => {
...
async function doWebSocketTest() {
  ...
  try {
    ...
    const EDITED_COMPOSITION = compositions.guestCompositions.find((c) => c.title === 'Compo Member 1 - 2');
    ...
    const WSTestPromise = new Promise((resolve, reject) => {
      ...
      const testSteps = {step: 0} //0: start, 1: guest created an element, 2: I move the element, 3: guest moved the element, 4: I delete the element
      const client = new Client({
        ...
        onConnect: async () => {
          ...
          await waitSeconds(2);
          sendFirstElementCreationOrder(client, EDITED_COMPOSITION.id, testSteps);
        },
